...
- **현재 상태**: 초기화 단계 (기술 스택 및 소스 코드 구성 진행 중)
...
> 기술 스택과 툴링이 확정되면 아래 항목을 업데이트하세요.
...
## 3. 핵심 아키텍처: 하이브리드 탐지 파이프라인

Claude는 코드 생성 시 아래 **4단계 프로세스**를 반드시 준수해야 합니다.

### 3-1. Pre-detection (프론트엔드)
...
Claude가 코드를 작성할 때 참고할 기술적 제약 사항입니다.

### Frontend (Plugin)
- 화면 캡처 및 Overlay UI 구현 시 **성능 최적화(FPS 저하 방지)**에 집중할 것.
- 노년층을 고려하여 **큰 폰트, 높은 대비, 직관적인 아이콘**을 사용할 것.

### Backend (Server)
- **FastAPI** 또는 유사한 비동기 프레임워크 권장.
- `Vector DB`는 고속 검색이 가능한 **Milvus**, **Pinecone**, 또는 **ChromaDB** 고려.
...
### 문서화
- 모든 함수와 클래스에는 **Google Style Docstring**을 포함할 것.
...
- 사용자가 혼란을 느끼지 않도록 에러 상태를 절대 날것으로 노출하지 않을 것.
